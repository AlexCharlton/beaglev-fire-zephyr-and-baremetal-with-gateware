 ...
 */
        ...
        uint32_t add_zlp)
    {
        ...
        {
            sprintf(info_string, "fifo_size %d max_pkt_size %d\n\r", fifo_size, max_pkt_size);
            MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)info_string);
            if ((max_pkt_size * 2) <= fifo_size)
            {
                txep_ptr->dpb_enable = DPB_ENABLE;
                MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)"DPB ENABLE\n\r");
            }
            else
            {
                MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)"DPB DISABLE\n\r");
    ...
    */
        ...
        uint8_t status)
    {
        ...
        char info_string[200];
        sprintf(info_string, "ep_tx_complete_cb %d status %d\n\r", num, status);
        MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)info_string);
        ...
        else
        {
            ...
            if (DMA_ENABLE == txep_ptr->dma_enable)
            {
                ...
                sprintf(info_string, "DMA IRQ %d CTRL %d ADDR %d COUNT %d Incremented ADDR %d\n\r", USB->DMA_CHANNEL[txep_ptr->dma_channel].IRQ, USB->DMA_CHANNEL[txep_ptr->dma_channel].CNTL, USB->DMA_CHANNEL[txep_ptr->dma_channel].ADDR, USB->DMA_CHANNEL[txep_ptr->dma_channel].COUNT, increamented_addr);
                MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)info_string);
            ...
            {
                sprintf(info_string, "DMA DISABLE\n\r");
                MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)info_string);
        ...
        else
        {
            ...
            MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)"tx not complete\n\r");
    ...
    */
        ...
        uint32_t dma_addr_val)
    {
        ...
        else
        {
            ...
            {
                MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)"DMA CB\n\r");
                ...
                {
                    MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)"DMA set_txpktrdy\n\r");
