    ...
    uint8_t usb_dma_plic_IRQHandler(void)
    {
        ...
        while (dma_irq)
        {
            if (dma_irq & MSS_USB_BYTE_BIT_0_MASK)
            {
                ...
                sprintf(info_string, "DMA IRQ %d COUNT %d\n\r", dma_channel, USB->DMA_CHANNEL[dma_channel].COUNT);
        ...
        if (ep_num && (buf_addr != 0))
        {
            if (DMA_ENABLE == dma_enable)
            {
                ...
                else
                {
                    ...
                    sprintf(info_string, "\nDMA WRITE pkt COUNT %d (actual %d) addr %d\n\r", txn_length, USB->DMA_CHANNEL[dma_channel].COUNT, buf_addr);
                    MSS_UART_polled_tx_string(&g_mss_uart0_lo, (const uint8_t *)info_string);
                }
