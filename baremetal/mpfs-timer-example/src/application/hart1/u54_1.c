...
uint8_t g_message2[] =
    "\r\r\nObserve the messages on uart0 terminal .\
...
void u54_1(void)
{
    ...
    /* set priorities */
    PLIC_SetPriority(MMUART0_PLIC_77, 1);
    ...
    MSS_UART_polled_tx(&g_mss_uart0_lo, g_menu, strlen(g_menu));
    MSS_UART_polled_tx(&g_mss_uart0_lo, g_message2, strlen(g_message2));

    /* uart0 Interrupt based menu which will change the Timer configurations.*/
    while (1u)
    {
        if (g_rx_size > 0u)
        {
            ...
            case '1':
                ...
                sprintf(p_buff, "Timer is configured in PERIODIC_MODE \
                            \r\nPlease observe the uart0 terminal for interrupt messages\r\n\r\n ");
                MSS_UART_polled_tx(&g_mss_uart0_lo, p_buff, strlen(p_buff));
